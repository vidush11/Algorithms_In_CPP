...
//    cout<<"ep-"<<ep<<" key-"<<key<<" comparing to "<<node->key<<" gives: ";
//    print_CRT(cr);
    if (cr==CRT::before){ found= del(key, node->left) ;}
    else if (cr==CRT::after){ found= del(key, node->right); }
    else { //cr==CRT::match
...
//                cout<<"Event Point "<<setprecision(15)<<ep_<<endl;
                
//                if (ep_==Point(1823, 1343)){ cout<<status_queue->left->left->left->left->left->left->left->left->left->left->left->left->left->left->right->right->key;
//                }
                
                for (LineSegment line: other){
                    del(line, status_queue);
//                    cout<<line.upper<< "; "<<line.lower<<"deleted"<<endl;
...
//                if (ep_==Point(1823, 1343)){
//                    vector<LineSegment> does_it_contain= search_matching(ep_, status_queue);
//                    for (LineSegment line:does_it_contain){
//                        if (line.lower==Point(1823, 1343)) {
//                            cout<<line;
//
//                            cout<<status_queue->left->left->left->left->left->left->left->left->left->left->left->left->left->left->right->right->key;
//                        }
//                    }
//                }
            ...
            for (int i=0; i<u; i++){
//                cout<<"inserting "<< upper[i]<<endl;
                insert(upper[i], status_queue);
            }
            for (int i=0; i<c; i++){
//                cout<<"inserting "<< contain[i]<<endl;
...
//    Point ep__=Point(782, 1343);
//    LineSegment to_compare=LineSegment(Point(1810, 1345), Point(1823, 1343));
//    LineSegment key=LineSegment(Point(1816, 1350), Point(1839, 1327));
//    
//    print_CRT(status_comparator(ep__, to_compare, key));
